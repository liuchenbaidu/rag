...
def tokenize_and_remove_stopwords(tokenizer, text, stopwords):
    if not text:
        logger.warning(f"Empty text received for tokenization")
        return []
    words = tokenizer.cut(text)
    filtered_words = [word for word in words
                      if word not in stopwords and word != ' ']
    if not filtered_words:
        logger.warning(f"No words remained after filtering. Original text: {text[:200]}...")
...
class BM25Retriever(BaseRetriever):
    def __init__(
        ...
        if not nodes:
            raise ValueError("No nodes provided for BM25 indexing")
        ...
        # Process corpus with detailed logging
        self._corpus = []
        logger.warning(f"Processing {len(nodes)} nodes for BM25 indexing...")
        
        for i, node in enumerate(self._nodes):
            # Get document path and other metadata
            doc_path = node.metadata.get('file_path', 'Unknown path')
            ...
            if not content:
                ...
                logger.warning(f"File name: {doc_name}")
                logger.warning(f"Page/Section: {page_label}")
                logger.warning(f"Full metadata: {node.metadata}")
                continue
                
            tokens = tokenize_and_remove_stopwords(self._tokenizer, content, stopwords=stopwords)
            if not tokens:
                logger.warning(f"No tokens generated for node {i}")
                logger.warning(f"Document: {doc_path}")
                logger.warning(f"File name: {doc_name}")
                logger.warning(f"Page/Section: {page_label}")
        ...
        empty_docs = len([c for c in self._corpus if not c])
        logger.warning(f"Found {empty_docs} empty documents out of {len(self._corpus)} total documents")
            
        if not any(self._corpus):
            raise ValueError("All documents are empty after processing")
